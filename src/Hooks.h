...
namespace Crucible
{
	...
	public:
		...
		struct BlockHitHook
		{
			...
			{
				//RE::DebugNotification("Block hit");
				//return 0;
				return func(actor);
			}
		...
		struct RegularRecoilHook
		{
			...
			{
				RE::DebugMessageBox("Regular Recoil")
				return func();
			}
		...
		static void Install()
		{
			...
			_NotifyAnimationGraph = Player_AnimationGraphManagerHolder_Vtbl.write_vfunc(0x1, NotifyAnimationGraph);
			...
			_CharacterUpdate = Character_Main_Vtbl.write_vfunc(0xAD, ActorUpdate);
			_PlayerUpdate = Player_Main_Vtbl.write_vfunc(0xAD, ActorUpdate);
	...
	private:
		...
		{
			RE::DebugMessageBox("No equippy mmm mmm no siree");
			//return true;
		...
		{
			RE::DebugMessageBox("double no equippes");
			//return true;
			if (a_actor)//This is done to preserve the functionality to get to that point.
		...
		{
			if (a4)
				RE::DebugMessageBox(std::format("Hit: {}", a4->GetName()));
		...
		static RE::BSFadeNode* PreTest(void* a1, RE::Projectile* a2, RE::BGSProjectile* a3, std::int64_t a4, float a5)
		{
			...
			{
				if (collidable)
					RE::DebugMessageBox(std::format("hit on num {}", i));
			}
		...
		static void OnSweepHit(RE::Actor* hit_causer, RE::Actor* hit_target, std::int64_t a_int1, bool a_bool, void* a_unkptr) {
			...
			RE::DebugNotification("Extra targets not in combat");
		}
		...
		static std::int64_t ActorUpdate(RE::Actor* a_this, float a2, void* a3);
		
		
		static void ProcessHitEvent(RE::Actor* a_actor, RE::HitData& a_hitData, float* a3);
		...
		static inline REL::Relocation<decltype(ActorUpdate)> _CharacterUpdate;
		static inline REL::Relocation<decltype(ActorUpdate)> _PlayerUpdate;
