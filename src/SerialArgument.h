...
namespace Crucible
{
	...
	public:
		...
		SerialArgument(SKSE::SerializationInterface* enter_face, SerializingState state)
		{
			...
			{
				uint32_t value_dmp = 0;
				if (serialInterface->GetNextRecordInfo(value_dmp, interfaceVersion, value_dmp) == false){
					//Finish, log error.
				}
			}
			break;
		...
		Type GetSerialized(Type& target, bool& success, Type* pre_deserialize = nullptr)
		{
			static_assert(
				...
				"At least serialization function of ISerializable must be overloaded (not overriden).");
			...
			if (!pre_deserialize)
			{
				if (!original_lock)	//If this is the first serializable object.
				{
					...
					serial_obj.HandleSerialize(*this, success, &target);
					serial_obj.OnSerialize(*this, success, &target);
				...
				else
				{
					...
					target.HandleSerialize(*this, success, &target);
					target.OnSerialize(*this, success, &target);
					
					return target;
				}
			}
			
			target.HandleSerialize(*this, success, pre_deserialize);
			target.OnDeserialize(*this, success, pre_deserialize);
		...
		//*
		template <class Type>requires(!std::is_pointer_v<Type>)
		bool Serialize(Type& target, std::uint32_t intro_version = 0, std::uint32_t incompatible_version = 0, SerializingFlag flags = SerializingFlag::None)
		{
			...
			if constexpr (std::derived_from<Type, ISerializer> == true)
			{
				...
				{
					uint32_t temp_typeCode = REPLACE_TEST_VAR;
					
					if (IsSerializing() == true) {
						serialInterface->OpenRecord(REPLACE_TEST_VAR, split_v_process(sizeof(Type), original_depth));
					}
					else if (IsDeserializing() == true) {//But if it's deserializing I want to make sure the types match.
						//Clean this up.
						if (temp_typeCode != _processData.type) {
							logger::info("ERROR: process typecode {/b} does not equal current depth {/b}.", _processData.type, temp_typeCode);
			...
			else if (IsSerializing() == true)
			{
				...
				if (return_value && !original_lock)
				{
					...
					if ((flags & SerializingFlag::SerializeAsData) != SerializingFlag::None)
						return_value = serialInterface->WriteRecordData(serialObject);
						...
						return_value = serialInterface->WriteRecord(REPLACE_TEST_VAR, split_v_process(sizeof(Type), original_depth), serialObject);
			...
			else if (IsDeserializing() == true)//Getting rid of this, temporarily just to see if it'll work without it
			{
				...
				//if (!original_lock && true == false) {
					...
					logger::info("Value Deserialize Failure for {}.", typeid(deserialized).name());
		...
		//*/
		template <class PointerType>requires(std::is_pointer_v<PointerType>)
		bool Serialize(PointerType target, std::uint32_t intro_version = 0, std::uint32_t incompatible_version = 0, SerializingFlag flags = SerializingFlag::None)
		{
			...
			return Serialize(*target, intro_version, incompatible_version, flags);
		}
...
/*Old Serialize function.
		...
		bool Serialize(Type* target, std::uint32_t intro_version = 0, std::uint32_t incompatible_version = 0, bool deprecated = false)
		{
			...
			// This should actually throw an error when serializing, to symbolize, while you can compile that, it will not allow you to serialize
			// a pointer, and instead it needs to be handled in some way.
			Serialize(target, intro_version, incompatible_version, deprecated);
		}
